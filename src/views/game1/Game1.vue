...
<!--      <img src="~@/assets/img/dog.png" alt="" @click="clickItem" :style="{'left': random.x + 'px', 'top': random.y + 'px'}">-->
    ...
    <div class="scoped">
      <div class="set">
        ...
        <div class="btn">
          <radius-click class="radius-click" @click.native="playGame" :message="message"></radius-click>
          <radius-click class="radius-click" @click.native="endGame" message="结束游戏"></radius-click>
        </div>
      </div>
      <div class="count">
        <h3>统计分数面板</h3>
...
export default {
  ...
  methods: {
    ...
    playGame () {
      ...
      if (!this.timerId) {  //当定时器为null，开始游戏
        this.timerId = setInterval(() => {
          if (this.count < this.TOTAL) {  //当地鼠出现的次数小于总数时，开始游戏
            this.count++
